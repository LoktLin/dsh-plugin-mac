...
**默认建议走 A 起步、确有必要再升 B。** 理由：A 没有构建链，改完刷新页面就能看到结果，反馈循环最短；等你要发布、或者发现绕不开某个官方接缝（比如必须出现在设置页、必须进入对话区），再迁 B —— 那时你已经明确知道需要哪个槽位了。

> **A 路线的完整参考源码**（29 个工具 + 面板，本技能逐层解剖的对象）在
> 本机 `参考-源码/dsh-eggy` —— **只读参照、未安装**；仓库在 [dsh-eggy](https://github.com/LoktLin/dsh-eggy)。
> ⚠️ 它依赖的内核技能（`lua-eggy-dsh`）不在本机，所以**读代码可以，跑不起来**。

### 📁 本机约定：插件源码一律写在 `参考-源码/` 下

本技能是**本机专用**（macOS 单机），插件源码不再散落到别处：
**新写的插件包放 `~/.dsh/skills/dsh-plugin-mac/参考-源码/<包名>/`。**
...
- 脚手架默认就生成到这里：`node scripts/scaffold.mjs dsh-mytool`（默认 `--dir` 已指向 `参考-源码/`）；
- `参考-源码/` 下**不只有只读参照**——它是本机插件的**源码根目录**，活代码也在这里；
- 已安装的插件用 `~/dsh-plugins/<包名>` 软链指回来，profile 的安装记录保持不动（见 §2 第 5 步）；
- 新增源码目录后，**更新上面的清单**，并在 `references/ecosystem.md` 里登记一句它是什么。
...
1. **本机类型定义是最好的文档**（比 README 准，且一定与运行版本一致）：
   ```bash
   # 全局本体（nvm 装法）：$(npm root -g)/@deepseek-ai/dsh
   BASE="$(npm root -g)/@deepseek-ai/dsh/node_modules/@deepseek-ai"
   grep -rl --include='*.d.ts' 'interface SlotMap' "$BASE"      # 谁声明了槽位
   ls "$BASE/dsh-host-webserver/lib/types"
   ```
...
```bash
node ~/.dsh/skills/dsh-plugin-mac/scripts/scaffold.mjs dsh-mytool          # 默认落在 参考-源码/dsh-mytool
node ~/.dsh/skills/dsh-plugin-mac/scripts/selftest.mjs --plugin ~/.dsh/skills/dsh-plugin-mac/参考-源码/dsh-mytool
```
...
> **`selftest` 对"手写插件"同样可用**：它认识 `package.json` 的 `main`/`exports`（入口放 `lib/` 也行），
> 认 `ctx.effect` 注册的回收器（`apply` 不必返回 cleanup），本机守卫**Host 头与 `remoteAddress` 任一**通过即可。
> 少数**脚手架专属约定**（`/status` 里的 `version`/`tools` 自述字段、`POST /<base>/tool` 调试端点）在缺失时标 `➖ 跳过`，
> **不算失败**——官方契约里没有这几项。
...
>
> 只把包塞进 `node_modules`（软链或拷贝）而这两件事都没做 = 包在盘上、`apply()` 从未执行、**不报任何错**。
> 本机（macOS）实证对照：`dsh-skill-manager` 出现在 profile 的 `bundles` 列表里；
...
# - insert:
#     - id: mytool
#     name: '/绝对路径/dsh-plugins/dsh-mytool/index.js'   # name 可以是**文件绝对路径**（源码直跑）
#                                                        # ⚠️ YAML 不展开 ~ / $HOME，必须写绝对路径
...
```bash
ln -s ~/dsh-plugins/dsh-mytool ~/.dsh/profiles/web/node_modules/dsh-mytool
...
| `cordis.patch.yml`（patch 文件） | **取决于 `patchReload`**：`live` → **热加载，不用重启**；`startup` → 只应用一次，要重启。本机 profile **未显式声明**该字段 → 按官方默认 **`live`** |
...
node ~/.dsh/skills/dsh-plugin-mac/scripts/selftest.mjs --plugin <插件目录>

# ② 组合期：确认插件真的进了配置树（不启动也能查）
dsh --profile web --dump-config | grep "dsh-mytool"

# ③ 路由通不通（Host 半边活着）
curl "http://127.0.0.1:3080/mytool/status"
...
**交付前的三件事**（L1–L3 全绿也**必须**做，原因见 `references/pitfalls.md` E6）：

1. **让真宿主调一次工具**（不是脚本 `import` 后直接 `execute`）—— 这一条能抓出 lossless 之类的**通道差异**；
2. **用 symlink 路径跑一次 CLI**（`node <软链>/插件.mjs`）—— 抓入口守卫静默失败（C6）；
3. **面板点一遍**（刷新 → 点入口 → 切 Tab）—— 抓渲染期问题。

> 做不到时**在交付说明里标"未验证"**，别写"完成"。这三件事最容易因为"真机麻烦"被跳过，
> 而实测里**大部分"桩全绿、真机挂"都出在这三处**。
...
| 无 shell | 同上 | 给出等价的手工命令（`ln -s` / `dsh plugin add`）让用户自己跑 |
...
| `references/pitfalls.md` | 踩到坑 / 提交前自检。分七类：**0 装配期**（症状最先查这里）、**A 静默失效**、**B 壳起不来**、**C 改了不生效**、**D DOM 注入路线**、**E 开发流程**（桩绿真机红、自检污染…）、**F 写面板（槽位路线）** |
...
**别猜，跑探针。** 本技能里的契约以**本机当前的 `dsh 0.1.7-rc.1`** 为准
（历史取证自 `0.1.2-rc.1`，已在 `0.1.7-rc.1` 上复核：探针 **10 ok / 0 需要处理**）——**本体会升级，这些具体值会过期**。
升级后第一件事：

```bash
node ~/.dsh/skills/dsh-plugin-mac/scripts/probe-contracts.mjs --scan "<你的插件目录>"
```
...
> 改完探针/文档后跑 `node ~/.dsh/skills/dsh-plugin-mac/scripts/test-skill.mjs` 自检。
