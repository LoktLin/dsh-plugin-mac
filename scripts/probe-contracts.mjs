...
function findDshCore() {
  ...
  // macOS 主流装法：nvm / Homebrew / 官方 pkg 都是 <node>/bin/node → <node>/lib/node_modules
  cands.push(path.join(nodeDir, '..', 'lib', 'node_modules', '@deepseek-ai', 'dsh'));
  ...
  try {
    const root = String(execFileSync('npm', ['root', '-g'], { encoding: 'utf8', timeout: 20000 })).trim();
...
/** 8. profile 侧：装配清单与 patch 热加载语义 */
{
  ...
  if (!j) bad('profile', 'profile 的 dsh.profile 段', '读不到 ' + prof);
    ...
    if (!Array.isArray(bundles)) bad('profile', 'profile 的 dsh.profile.bundles', '不再是数组（装配清单语义可能变了）', 'changed');
      ...
      // 官方 app-boot 明确：custom profile **省略** reload 策略时保留历史默认值 live
      // （随产品交付的 web 模板同样是 live）。所以"没写"不等于"判断依据没了"——
      // 早先按缺失判 changed 是**误报**，会让每台自定义 profile 都无法存基线。
      WARNINGS.push('profile 未显式声明 patchReload —— 按官方默认值 live 处理（custom profile 省略时默认 live）。');
      ok('profile', 'profile 装配清单与 patchReload', `bundles ${bundles.length} 项；patchReload 未显式声明 → 官方默认 live`, { data: { bundles, patchReload: 'live' } });
