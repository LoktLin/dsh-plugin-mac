#!/usr/bin/env node
/**
 * dsh-plugin-mac 技能「契约探针」部分的自动化测试跑手（原 dsh-core-update 技能，2026-09-12 并入；macOS 专用）
 *
 * 被测对象：探针与其文档（`~/.dsh/skills/dsh-plugin-mac`）——
 ...
 *
 * 环境：macOS。不依赖 Python（解释器路径与版本差异大），统一用 node 断言。
 * 用法：
...
const SKILL = path.join(os.homedir(), '.dsh', 'skills', 'dsh-plugin-mac');
...
const j3 = await runProbe(['--scan', '/__definitely_not_exists__', '--json']);
...
const j7 = await runProbe([], { DSH_CORE_DIR: '/__no_such_dsh__' });
...
//      一旦 SKILL.md 里提到**别的工程**的脚本（例如 `<蛋仔面板工程>/scripts/test-entry-anchors.mjs`），
//      它也会被当成技能自带的 → 报"不存在" → **假警报**。
//      而假警报会训练人忽略测试，比没有检查更糟。
//   ⇒ 现在按**路径形态**分流：裸 `scripts/x.mjs` 才相对技能目录；
//      以 `/` 开头（绝对路径）或 `~/` 开头（家目录）的按那个绝对路径校验；
//      带 `<...>` 的占位符路径**不参与**存在性断言（本机没有那个工程，断言它存在只会制造假警报）。
//      排除集里带上 `=` / `$`：否则文档里的 shell 赋值（`P=~/…/probe-contracts.mjs`）
//      会被整段吃进路径，`P=` 前缀让绝对路径被误判成相对路径 → 又一条假警报。
...
const jEnv = await runProbe(['--json'], { DSH_CORE_DIR: '/__not_a_dsh_core__' });
...
if (AS_JSON) {
  ...
  console.log('');
  console.log('===== dsh-plugin-mac·契约探针测试（原 dsh-core-update） =====');
  console.log('');
  for (const r of R) {
