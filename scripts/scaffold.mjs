 ...
 *   node scripts/scaffold.mjs dsh-mytool --dir <目录> --title "我的工具面板"
 *   node scripts/scaffold.mjs dsh-mytool --json
 *
 * 默认输出目录是本技能的 `参考-源码/`（本机约定：插件源码统一放那里，见 SKILL.md §1）。
 *
...
// 本机约定：插件源码统一落在本技能的 `参考-源码/` 下（SKILL.md §1）。
// 显式 --dir 仍然优先，方便临时在别处试。
const outRoot = path.resolve(val('dir', path.join(HERE, '..', '参考-源码')));
...
const nextSteps = [
  ...
  '④ 路由连通性：curl "http://127.0.0.1:3080' + PREFIX + '/status"',
];
