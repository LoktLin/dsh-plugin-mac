...
function check(name, fn) {
  return Promise.resolve()
    ...
    .catch((e) => {
      if (e && e.__skip) results.push({ name, ok: true, skipped: true, detail: '跳过：' + e.message });
      else results.push({ name, ok: false, detail: String((e && e.message) || e) });
    });
}
...
function fakeReq(method, url, { host = '127.0.0.1:3080', body = null, remoteAddress = '127.0.0.1' } = {}) {
  const handlers = {};
  const req = {
    method, url, headers: { host },
    // 真实 http.IncomingMessage 一定有 socket.remoteAddress —— 桩也得给，
    // 否则带 loopback 守卫的插件（`req.socket.remoteAddress === '127.0.0.1'`）会被一律 403 误杀。
...
async function loadHostHalf(pluginDir) {
  // 入口不一定是根的 index.js —— 尊重 package.json 的 main/exports（真实插件常把入口放 lib/ 下）。
  let entry = path.join(pluginDir, 'index.js');
  try {
    const pkg = JSON.parse(fs.readFileSync(path.join(pluginDir, 'package.json'), 'utf8'));
    const main = pkg.main
      ?? (typeof pkg.exports === 'string' ? pkg.exports : pkg.exports?.['.']?.default ?? pkg.exports?.['.']);
    if (typeof main === 'string' && main) entry = path.resolve(pluginDir, main);
  } catch { /* 没 package.json 或解析失败就按根 index.js */ }
  if (!fs.existsSync(entry)) {
    const alt = path.join(pluginDir, 'lib', 'index.js');
    if (fs.existsSync(alt)) entry = alt;
  }
  assert(fs.existsSync(entry), '缺 Host 入口（index.js 或 package.json.main 指向的文件）：' + entry);
...
/**
 * 解析 Host 入口：优先 `package.json` 的 main/exports[.]，退回根 index.js、再退 lib/index.js。
 * 脚手架产物入口在根，但**手写插件常把入口放 lib/ 下** —— 硬编码根 index.js 会误判。
 */
function hostEntryOf(dir) {
  let entry = path.join(dir, 'index.js');
  try {
    const pkg = JSON.parse(fs.readFileSync(path.join(dir, 'package.json'), 'utf8'));
    const main = pkg.main
      ?? (typeof pkg.exports === 'string' ? pkg.exports : pkg.exports?.['.']?.default ?? pkg.exports?.['.']);
    if (typeof main === 'string' && main) entry = path.resolve(dir, main);
  } catch { /* 按默认 */ }
  if (!fs.existsSync(entry)) {
    const alt = path.join(dir, 'lib', 'index.js');
    if (fs.existsSync(alt)) entry = alt;
  }
  return entry;
}
...
try {
  await check('脚手架产物齐全', () => {
    const entry = hostEntryOf(pluginDir);
    const need = ['package.json', 'cordis.patch.yml', path.relative(pluginDir, entry), path.join('lib', 'client.js'), 'README.md'];
    const missing = need.filter((f) => !fs.existsSync(path.join(pluginDir, f)));
    assert(missing.length === 0, '缺文件：' + missing.join(', '));
    return need.length + ' 个文件（Host 入口 ' + path.relative(pluginDir, entry) + '）';
  });
  ...
  await check('两个半边都是合法 JS 语法', async () => {
    const entryRel = path.relative(pluginDir, hostEntryOf(pluginDir));
    for (const f of [entryRel, path.join('lib', 'client.js')]) {
      await new Promise((resolve, reject) => {
        execFile(process.execPath, ['--check', path.join(pluginDir, f)], { timeout: 30000 },
  ...
  await check('Host: GET ' + base + '/status 回 {ok:true,data}', async () => {
    ...
    // version/tools 是**脚手架约定的自述字段**，官方契约并不要求业务插件提供。
    // 有就校验，没有就只算「信封正确」——否则真实插件会被这条误判为失败。
    if (typeof j.data.version !== 'string' || !Array.isArray(j.data.tools)) {
      return '信封正确（本插件未提供 version/tools 自述字段——脚手架约定项，跳过其形状校验）';
    }
    assert(j.data.tools.length >= 2, 'data.tools 应为工具名数组');
  ...
  await check('Host: POST ' + base + '/tool 能按名调用工具并回显', async () => {
    ...
    // `/<base>/tool` 是脚手架的**可选**通用调试端点，官方契约没要求业务插件实现它。
    if (r.status === 404) skip('本插件没实现 ' + base + '/tool 调试端点（脚手架可选约定，非官方契约）');
    if (!toolName) skip('本插件没有 *_echo 工具（脚手架约定名），无法按名回显');
    assert(r.status === 200, 'HTTP ' + r.status + '：' + r.text.slice(0, 200));
    const j = JSON.parse(r.text);
  ...
  await check('Host: 非本机来源被 403 拒绝', async () => {
    // 本机守卫有两种常见写法，**任一拦下即可**：
    //   ① 看 socket.remoteAddress（推荐，无法伪造）
    //   ② 看 Host 头
    const byHost = await callRoute(route.handler, 'GET', base + '/status', { host: 'evil.example.com' });
    const byAddr = await callRoute(route.handler, 'GET', base + '/status', { remoteAddress: '10.9.9.9' });
    assert(byHost.status === 403 || byAddr.status === 403,
      '非本机来源没被拦截（Host 头 → ' + byHost.status + '，remoteAddress → ' + byAddr.status + '）：面板数据会暴露给同网段');
    return byAddr.status === 403 ? 'remoteAddress 守卫' : 'Host 头守卫';
  });
  ...
  await check('Client: cleanup 把副作用全部收回，且回收后能重挂（热重载不留幽灵）', () => {
    // 两种写法都算数：
    //   ① apply 直接返回 cleanup —— 脚手架产物的写法；
    //   ② 用 ctx.effect(fn, label) 注册 —— **官方推荐**，apply 不必返回值。
    const disposers = [];
    if (typeof firstCleanup === 'function') disposers.push(firstCleanup);
    if (firstSt) {
      for (const e of firstSt.effects) if (typeof e.dispose === 'function') disposers.push(e.dispose);
    }
    assert(disposers.length > 0,
      'apply 既没返回 cleanup 函数，也没通过 ctx.effect 注册任何可回收副作用 —— 热重载会留下幽灵入口/面板');
    for (const d of disposers) { try { d(); } catch { /* 单个 disposer 失败不阻断回收检查 */ } }
    if (clientRoute === 'dom') {
...
if (AS_JSON) {
  ...
  console.log('');
  console.log('=== dsh-plugin-mac 脚手架自检    插件：' + pluginDir + ' ===');
  console.log('');
  for (const r of results) console.log((r.skipped ? '➖' : r.ok ? '✅' : '❌') + ' ' + r.name + (r.detail ? '  → ' + r.detail : ''));
  console.log('');
  console.log('合计：' + passed + ' 项 ✅ / ' + failed + ' 项 ❌' + (skipped ? '（其中 ' + skipped + ' 项为「不适用，已跳过」）' : ''));
}
