...
> 本文件是 `dsh-plugin-mac` 技能的「本体升级」部分（原独立技能 `dsh-core-update`，2026-09-12 并入）。
...
node ~/.dsh/skills/dsh-plugin-mac/scripts/probe-contracts.mjs

# 想知道"是不是真升级了"：
cat ~/.dsh/profiles/web/package.json                              # 看 dsh / dsh-* 的版本
stat -f '%Sm  %N' "$(npm root -g)/@deepseek-ai/dsh/package.json"  # 本体安装时间（macOS stat 语法）
```
...
```bash
P=~/.dsh/skills/dsh-plugin-mac/scripts/probe-contracts.mjs
node "$P"                              # 人类可读（不给参数 = 自动发现使用面）
node "$P" --json                       # 机器可读
node "$P" --no-scan                    # 明确不要使用面（只想知道"契约变没变"时）
node "$P" --scan ~/dsh-plugins/<你的插件> ...  # 显式指定使用面（可给多个）
```
...
```bash
# 蛋仔插件的侧边栏入口锚点回归（A 组直接对照真实壳代码 = **升级预警**）
node <dsh-eggy 仓库>/scripts/test-entry-anchors.mjs

# ⚠️ 但该脚本的 A 组按 **Windows 的 npm 全局布局**定位 DSH 本体（两个候选都只写在 nodeDir 之下），
#    在 macOS 的 nvm/Homebrew 布局下两个候选都不成立 → DSH=null → **A 组被整个跳过**（只剩 B 组逻辑测试）。
#    临时替代：手工执行 A 组判据，把 ANCHORS 的类名子串拿去真实壳文件里搜：
#      DSH="$(npm root -g)/@deepseek-ai/dsh"
#      LAY="$DSH/node_modules/@deepseek-ai/dsh-client-ui-layout/lib/client.js"
#      SID="$DSH/node_modules/@deepseek-ai/dsh-client-ui-sidebar/lib/client.js"
#      for s in sidebarCol logoRow newSession regionArea footArea; do grep -c "$s" "$LAY" "$SID"; done
#    ✅ 2026-09-19 实测（DSH 0.1.5-rc.2）：**5 个锚点全部命中，`data-sidebar-collapsed` 也在**——
#       从 0.1.2-rc.1 那次"掉了两个"之后没有再掉。以这条结论为准，别再引用 2026-09-11 的旧记录。
```
...
```bash
node ~/.dsh/skills/dsh-plugin-mac/scripts/test-skill.mjs          # 人类可读
node ~/.dsh/skills/dsh-plugin-mac/scripts/test-skill.mjs --json   # 机器可读
```
