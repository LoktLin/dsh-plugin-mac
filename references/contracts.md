...
> **`@deepseek-ai/dsh` `0.1.7-rc.1` · 复核于 2026-09-24**。
> ⚠️ **本机当前即 `0.1.7-rc.1`**（macOS）——但**本体会升级**，具体值必须在你的机器上按 §0 重新取证；
> 槽位计数这类东西尤其容易过期（实测两次跳变：**52 → 61 → 86**，最近一次伴随 26 项新增 / 1 项移除）。
> 探针（`scripts/probe-contracts.mjs`）会把槽位增删如实报出来，别拿旧值硬套。
...
grep -rl --include='*.d.ts' 'interface SlotMap' "$BASE"

# ② 某个槽位的 kind / scope / owner props 精确契约
cat "$BASE/dsh-client-ui-sidebar/lib/types/client/contract/slots.d.ts"

# ③ 服务与事件的契约
cat "$BASE/dsh-host-webserver/lib/types/index.d.ts"
cat "$BASE/dsh-client-ui-renderer/lib/types/client/registry.d.ts"
```
