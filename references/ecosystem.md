...
```bash
# 列出 profile 里所有已装包（含 @scope/xxx）
ls -d ~/.dsh/profiles/web/node_modules/@*/*/ ~/.dsh/profiles/web/node_modules/*/ 2>/dev/null
```

### `@linxin666/*` v0.3.23（本机实测）—— 同一作者的一整支（共 20 个包）
...
- `dsh-better-sidebar` v0.19.1 —— 侧边栏增强
- `dsh-skill-manager` v0.1.3 / `dsh-wechat` v0.9.6 / `dsh-context` v0.53.3 —— 本地自研/第三方小插件
- `dsh-digipet` v0.2.0 / `dsh-whale-widget` v0.3.5 / `@lyhalal/dsh-notification-center` v0.1.35 —— 桌宠与通知类

### 自研插件（本机源码放本技能的 `参考-源码/` 下 —— **该目录不进本仓库**）

| 包 | 形态 | 说明 |
|---|---|---|
| `dsh-lanhu` | **双面插件** | 蓝湖设计稿读取：Host 半边 13 个工具 + 6 段 HTTP 路由；Client 半边三 Tab 面板。**零依赖**（`puppeteer-core` 只在运行时动态 import）。源码约 3800 行 + 184 项离线自检 |

> **`dsh-lanhu` 值得当模板抄的地方**（手写 CJS + 官方槽位，零构建那条路）：
> · **本地 `tool()` 顶替 `defineTool`**（`link:` 安装下 import 不到官方包），两件事都做全：DSL→JSON Schema **+ execute 前参数校验**（见 pitfalls E5）；
> · **出口统一 `toLossless()` 清洗**（见 A1）+ **`ctx.effect` 回收** + **`inject: []` 不硬依赖**；
> · **CLI 入口守卫用 realpath 比较**（见 C6），带 stderr 兜底；
> · **`webServer` 路由 + `{ok,data}` 信封 + 本机来源守卫**（守卫用 `socket.remoteAddress`，比 Host 头可靠）；
> · **面板**：错误边界包每个 Tab、数据带稳定 uid 当 key、`localStorage` 全 try/catch（见 F 类）；
> · **自检**：`test/selfcheck.mjs`，用 `XXX_HOME` 环境变量隔离到临时目录（见 E7），纯离线秒级，把每个踩过的坑都固化成断言。

> 源码根目录约定见 `SKILL.md` §1：插件一律写在 `参考-源码/<包名>/`，
> 已安装的用 `~/dsh-plugins/<包名>` → 软链指回来，profile 的安装记录不动。

> ⚠️ **`dsh-eggy`（蛋仔面板）是只读参照，不是能跑的工程**：它原是本技能 Windows(Win10) 时期的实战工程，
> 本机那份 checkout 在 `参考-源码/dsh-eggy` 供读码；仓库见 [dsh-eggy](https://github.com/LoktLin/dsh-eggy)。
> **未安装、也别装** —— 它依赖的内核技能 `lua-eggy-dsh` 不在本机，跑不起来。
> 本技能拿它当 A 路线的教学范例（`lightweight-path.md` 有完整解剖）。
...
| 资源 | 内容 | 怎么用 |
|---|---|---|
| [NanmiCoder/dsh-agent-teams · `dsh-plugin-development`](https://github.com/NanmiCoder/dsh-agent-teams) | 一份**相当完整**的 DSH 插件开发 Skill（v3.1.1）：bundle/profile 契约、Service、工具、HTTP、持久化、slot 四步契约、Conversation Node、双 tsconfig、纯度门、分发与验证矩阵 | 最值得通读的外部资料。⚠️ **它自己在开头声明「主体保留 2026-08-13 的历史模板，不代表当前 Harness 的 API」**——所以把它当**索引和思路**，具体签名回本机 `.d.ts` 校验 |
...
| [Pasumao/dsh-plugin-dev-kb](https://github.com/Pasumao/dsh-plugin-dev-kb) | 「官方文档完整镜像 + 主题导航与检索」知识库 | 想快速检索官方文档时用 |
| [liangdabiao/dsh-plugin-developer-skill](https://github.com/liangdabiao/dsh-plugin-developer-skill) | 含 `references/browser-side.md`（Slots Toolview、沙箱与安全，基于 dsh 0.1.1-rc.2） | 浏览器侧专题；**注意版本号比本机旧** |
...
1. `grep -rl --include='*.d.ts' 'interface SlotMap' "$BASE"` → 得到**提供方清单**（`$BASE` 取值见 `contracts.md` §0）；
