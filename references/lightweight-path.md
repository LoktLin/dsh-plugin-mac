...
> 一个无构建插件（**29 个工具** + 一个面板），A 路线（纯 JS、零依赖、不打包）的完整样本。
> 下面每一段都来自实际源码，注释里的"为什么"是踩过之后补的。
>
> **源码位置**：[dsh-eggy 仓库](https://github.com/LoktLin/dsh-eggy)；本机那份 checkout 在 `参考-源码/dsh-eggy`（**只作只读参照、未安装**）。
> ⚠️ 它依赖的内核技能（`lua-eggy-dsh` 的 `scripts/lib/*.mjs`、`EGGY-SKILLS-MAP.md` 等）**不在本机**，
> 所以这份源码**能读、能静态核验结构，但跑不起来**——别指望 `node scripts/*.mjs` 全绿。
...
- **内核已经在技能里**：一整套现成实现在 `~/.dsh/skills/lua-eggy-dsh/scripts/lib/*.mjs`（旧路径 `lua-eggy/scripts/lib/` 只是指向它的兼容壳），插件只是把它们挂到 `tools` 上；
...
├── index.js              # Host 半边（1449 行）
├── lib/
│   ├── client.js         # Client 半边（1530 行，手写 CJS factory）
...
1. **无构建路线的上限在哪**：面板是手写 DOM + 手写 CSS 字符串数组，1530 行里大半是样式。
